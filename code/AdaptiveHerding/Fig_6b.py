...
"""
----------------------------------------------------------------------------
This file runs the simulations that yield to Figure 6b
...
xD = np.array([1.3, 0.5, -1.5, -0.9, -0.8, 1.1, 1.8, -0.4, 0.4, 0.9])
new_xD = [np.copy(xD) for i in range(N_herders)]

# Define initial position of the evaders 
x   = np.array([2.1, 0.7, -0.8, -1.4, -1.3, 1.8, 2.1, -1.0, 1.3, 1.5])
...
# Control mode
control_mode = np.ones(N_herders)
        ...
        if control_mode[i] == 0:
            ...
            for ii in range(M_evaders):
                for jj in range(M_evaders):
                    cost[ii,jj] = np.linalg.norm(x[2*ii:2*ii+2]-xD[2*jj:2*jj+2])
...
plt.show()
plt.savefig('odkf.eps', format='eps')