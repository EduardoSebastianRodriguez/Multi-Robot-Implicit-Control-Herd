...
%
% Evaders behave following: Inverse Model (2) and Exponential Model (1)
% --------------------------------------------------------------------------------
...
x_d = [0.45 0.15 -0.05 -0.25 -0.05 0.15]';

% Define initial position of the evaders 
x      = [0.9 0.5 -0.4 -0.4 0.0 0.0]';
x_plot = x;
x_prev = x;
...
theta_r   = [0.02;0.02;0.05];
theta_e   = [0.015;0.015;0.04];
        ...
        if k==1
            for j=1:M_evaders
                if j<3
                    ...
                    plot(x_d(2*j-1),x_d(2*j),'yo', 'MarkerSize',70,'MarkerFaceColor','y')
                    hold on
                end
            end
        end
...
for j=1:M_evaders
    if j<M_evaders % Inverse evaders
