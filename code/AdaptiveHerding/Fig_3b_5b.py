...
"""
----------------------------------------------------------------------------
This file runs the simulations that yield to Figures 3b, 4b and 5b
...
for k in range(Sim):
    ...
    tDot = -np.matmul(np.matmul(np.linalg.inv(np.matmul(np.transpose(Jt),Jt) + landa*np.eye(M_evaders)),np.transpose(Jt)),tD + np.matmul(Jx,f) + np.matmul(Ju,uDot))
