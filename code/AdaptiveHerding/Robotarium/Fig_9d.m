...
% --------------------------------------------------------------------------------
% 4 herders vs 4 evaders
%
% Evaders behave following: Inverse Model (2) and Exponential Model (2)
% --------------------------------------------------------------------------------
...
x_d = [0.45 0.25 -0.05 -0.25 -0.0 0.2 0.3 0.0]';

% Define initial position of the evaders 
x      = [0.9 0.5 -0.4 -0.4 0.0 0.0 0.5 0.1]';
x_plot = x;
x_prev = x;

% Define initial position of the herders
u        = [-1.4 0.0 0.0 -0.8 1.2 0.8 0.8 -0.8]';
u_plot   = u;
u_prev   = u;
...
X_max = 1.6-0.05; % From Robotarium documentation
X_min = -1.6+0.05;% From Robotarium documentation
Y_max = 1.0-0.05; % From Robotarium documentation
Y_min = -1.0+0.05; % From Robotarium documentation

% Define parameters of the evaders' model
theta_r   = [0.02;0.02;0.05;0.05];
theta_e   = [0.015;0.015;0.04;0.04];
...
final_goal_points = vertcat(reshape(horzcat(x',u'),[2,N_herders+M_evaders]),[-2.3 0.7 -0.7 -2.3 0.0 1.6 -2.3 2.3 ]);
    ...
    if flag ~= 0 
        ...
        for j=3:4
            ...
            for i=1:N_herders
                ...
                xdot(2*j-1:2*j) = xdot(2*j-1:2*j) + landa(j)*(x(2*j-1:2*j)-u(2*i-1:2*i))*exp(-(1/(sigmaS^2))*X);
            end
            if norm(xdot(2*j-1:2*j)) > v_max
                xdot(2*j-1:2*j) = v_max*xdot(2*j-1:2*j)/norm(xdot(2*j-1:2*j));
            end
        ...
        % Calculate Jacobians 
        Jx = buildJx(x,x_d,u,theta_e,K,betaS,tau,sigmaS);
        Ju = buildJu(x,x_d,u,theta_e,K,betaS,tau,sigmaS);
        ...
        theta_e = theta_e + T*tDot*0.00005;
        ...
        if k==1
            for j=1:M_evaders
                if j<3
                    plot(x_d(2*j-1),x_d(2*j),'ro', 'MarkerSize',30,'MarkerFaceColor','r')
                    hold on
                else
                    plot(x_d(2*j-1),x_d(2*j),'yo', 'MarkerSize',30,'MarkerFaceColor','y')
                    hold on
                end
            end
        end
...
function Jx = buildJx(x,xD,u,theta_e,K,betaS,tau,sigmaS) 
    ...
    for i=1:length(x)
        v        = zeros(length(x),1);
        v(i)     = epsilon;
        [A,B]    = buildSystem(x+v, u, theta_e,betaS,tau,sigmaS);
...
function Ju = buildJu(x,xD,u,theta_e,K,betaS,tau,sigmaS)
    % Preallocate Jacobian
    epsilon = 1e-6;
    ...
    for i=1:length(u)
        v        = zeros(length(u),1);
        v(i)     = epsilon;
        [A,B]    = buildSystem(x, u+v, theta_e,betaS,tau,sigmaS);
...
function J = buildJparams(x,u,betaS,tau,sigmaS)   
    % Preallocate Jacobian
    ...
    for j=3:4
        ...
        for i=1:N_herders
            ...
            J(2*j-1:2*j,j)  = J(2*j-1:2*j,j) + var*(x(2*j-1:2*j)-u(2*j-1:2*j))*exp(-1/sigmaS^2*X);
        end 
    end
end