...
"""
----------------------------------------------------------------------------
This file runs the simulations that yield to Figures 3a, 4a and 5a
...
for i in range(M_evaders):
    if i==0:
        ax1.plot(xD[2*i],xD[2*i+1],'ro',markersize=22,markerfacecolor='r',label='a')
    else:
        ax1.plot(xD[2*i],xD[2*i+1],'ro',markersize=22,markerfacecolor='r')
for i in range(N_herders):
    if i==0:
        ax1.plot(HP[2*i,0],HP[2*i+1,0],'g',marker=(5, 0, 180),markersize=14,markerfacecolor='g',label='a')
        ax1.plot(HP[2*i,-1],HP[2*i+1,-1],'mp',markersize=14,markerfacecolor='m',label='a')
        ax1.plot(HP[2*i,:],HP[2*i+1,:],'-.b',linewidth=4,label='a') 
    else:
...
for i in range(M_evaders):
    if i==0:
        ax1.plot(P[2*i,0],P[2*i+1,0],'gv',markersize=14,markerfacecolor='g',label='a')
        ax1.plot(P[2*i,-1],P[2*i+1,-1],'m^',markersize=14,markerfacecolor='m',label='a')
        ax1.plot(P[2*i,:],P[2*i+1,:],'k',linewidth=4,label='a') 
    else:
...
# Evolution of the estimates with time
fig4, ax4 = plt.subplots() 
for i in range(M_evaders):
    if i==0:
        ax4.plot(timeAxis,CR[i,:],'r--',linewidth=4, label="Real values ")
        ax4.plot(timeAxis,CE[i,:],'b',linewidth=4, label="Estimates ")
    else:
        ax4.plot(timeAxis,CR[i,:],'r--',linewidth=4)
        ax4.plot(timeAxis,CE[i,:],'b',linewidth=4)
plt.xlabel('time $[s]$') 
plt.ylabel(r'$ \theta $ $[m^3/s]$') 
ax4.set_xlim([0, t])
ax4.legend()
plt.show()